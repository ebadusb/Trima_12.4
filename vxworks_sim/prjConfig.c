...
void usrNetProtoInit (void)
    {
    ifIndexLibInit();                   /* Establishes unique index value for each IP device */
    ...
    netLibInit();			usrIfProxyArpInit(); /* creates the network task that runs low-level 			network interface routines in a task context */
