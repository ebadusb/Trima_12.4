...
#include <end.h>
#include "..\..\os\gei82543End.h"
#include "..\..\os\gei82543End.c"
...
LOCAL STATUS    sysGeiPciInterruptDisable(UINT32 networkAdapterBus, UINT32 networkAdapterDevice);
...
STATUS sys543PciInit
	...
	if ( geiUnits == 0 )
	{
		/* One time device initialization is needed to ensure that other PCI devices on this interrupt bus
		 * have interrupts disabled.
		 */
		sysGeiPciInterruptDisable(pciBus, pciDevice);
	}
...
LOCAL STATUS sysGeiPciInterruptDisable(UINT32 networkAdapterBus, UINT32 networkAdapterDevice)
{
   ...
   if ( pciConfigMech == PCI_MECHANISM_2 && PCI_MAX_DEV > 16 )
   {
	...
	for ( busNo=0; busNo <= 2; ++busNo )
	{
		...
		{
			pciConfigInWord (busNo, deviceNo, 0, PCI_CFG_VENDOR_ID, (UINT16 *)&vendorId);
			pciConfigInWord (busNo, deviceNo, 0, PCI_CFG_DEVICE_ID, (UINT16 *)&deviceId);
			...
			u.array[0] = 0;
			pciConfigInByte (busNo, deviceNo, 0, PCI_CFG_DEV_INT_LINE, &irq);
	
			if ( vendorId != 0xffff &&
				  irq == 10 &&
				  ( busNo != networkAdapterBus ||
					 deviceNo != networkAdapterDevice ) )
			{
				UINT16	cmd;
				pciConfigInWord(busNo, deviceNo, 0, PCI_CFG_COMMAND, &cmd);
				cmd |= 0x400;
				pciConfigOutWord(busNo, deviceNo, 0, PCI_CFG_COMMAND, cmd);
			}
		}
	}
