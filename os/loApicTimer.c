...
--------------------
01g,04sep02,hdn  stopped reseting the TSC to preserve it for other use.
...
LOCAL int     extraAuxClockRate               = 1024;
LOCAL int     extraAuxClockDivider            = 0;
LOCAL int     extraAuxClockSysCallbackEnabled = 0;
LOCAL FUNCPTR extraAuxClockRoutine            = NULL;
LOCAL int     extraAuxClockArg                = 0;
...
LOCAL UINT32  sysTimestampBase        = 0;                   /* timestamp at sys tick */
LOCAL void sysTimestampFreqGet (void);
LOCAL void sysTimestampRollover (void);
...
#ifdef  INCLUDE_TIMESTAMP_TSC
      sysTimestampRoutine = (FUNCPTR)sysTimestampRollover;
#endif  /* INCLUDE_TIMESTAMP_TSC */
...
void sysAuxClkInt (void)
{
   static int extraAuxClockCounter = 0;

   /* call auxiliary clock service routine */

   if (extraAuxClockRoutine != NULL)
      (* extraAuxClockRoutine) (extraAuxClockArg);

   if (sysAuxClkRoutine != NULL && extraAuxClockCounter <= 1 && extraAuxClockSysCallbackEnabled)
      (* sysAuxClkRoutine) (sysAuxClkArg);

   if (extraAuxClockCounter <= 1)
      extraAuxClockCounter = extraAuxClockDivider;
   else
      extraAuxClockCounter -= 1;
}
...
{
   extraAuxClockSysCallbackEnabled = 0;
}
...
void sysAuxClkEnable (void)
{
   ...
   extraAuxClockDivider            = extraAuxClockRate / sysAuxClkTicksPerSecond;
   extraAuxClockSysCallbackEnabled = 1;

   if (!sysAuxClkRunning)
   {
      tc = PIT_CLOCK / extraAuxClockRate;
...
void sysAuxClkEnable (void)
{
   ...
   if (!sysAuxClkRunning)
   {
      ...
      {
         if (auxTable [ix].rate == extraAuxClockRate)
         {
...
{
   return extraAuxClockRate;
}

STATUS extraAuxClockConnect (FUNCPTR routine, /* routine called at each aux clock interrupt */
                             int arg          /* argument to auxiliary clock interrupt routine */
                             )
{
   extraAuxClockArg     = arg;
   extraAuxClockRoutine = routine;
   sysAuxClkEnable();
   return (OK);
}
...
STATUS sysTimestampConnect (FUNCPTR routine, /* routine called at each timestamp timer interrupt */
                            int arg          /* argument with which to call routine */
                            )
{
   return (ERROR);
...
LOCAL void sysTimestampFreqGet (void)
{
   ...
   {
      _WRS_ASM ("rdtsc" : "=a" (sysTimestampFreqValue));
      sysTimestampFreqValue -= sysTimestampBase; /* ok for 32bit rollover */
      sysTimestampTickCount  = 0;
   }
   if (sysTimestampTickCount == 0)
      _WRS_ASM ("rdtsc" : "=a" (sysTimestampBase));
...
UINT32 sysTimestamp (void)
{
   ...
   _WRS_ASM ("rdtsc" : "=a" (value));
   value -= sysTimestampBase;   /* it handles the 32bit rollover case */
...
/*******************************************************************************
*
* sysTimestampRollover - emulate the timestamp rollover
*
* This routine emulates the timestamp rollover.  It sets the current value of
* the timestamp timer counter to the static variable sysTimestampBase that will
* be subtracted from the timestamp timer counter value obtained afterward in the
* sysTimestamp() and sysTimestampLock() routine.
*
* RETURNS: N/A
*
* SEE ALSO: sysTimestamp(), sysTimestampLock()
*/
...
/* FORMAT HASH aa0e542ef7cad74984be68f5695ff5cb */
