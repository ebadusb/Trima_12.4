...
LOCAL STATUS miiProbe (PHY_INFO*  pPhyInfo, /* pointer to PHY_INFO structure */
...
LOCAL STATUS miiPhyIsolate (PHY_INFO*  pPhyInfo, /* pointer to PHY_INFO structure */
...
LOCAL STATUS miiPhyPwrDown (PHY_INFO*  pPhyInfo, /* pointer to PHY_INFO structure */
...
LOCAL STATUS miiPhyBusScan (PHY_INFO*  pPhyInfo        /* pointer to PHY_INFO structure */
                            )
{
   UINT16 ix;       /* a counter */
...
LOCAL STATUS miiPhyBestModeSet (PHY_INFO*  pPhyInfo, /* pointer to PHY_INFO structure */
...
LOCAL STATUS miiPhyLinkSet (PHY_INFO*  pPhyInfo        /* pointer to PHY_INFO structure */
                            )
{
   UINT16 ix;       /* a counter */
   UINT8  phyAddr;  /* address of a PHY */
...
LOCAL STATUS miiPhyDefModeSet (PHY_INFO*  pPhyInfo        /* pointer to PHY_INFO structure */
                               )
{
   UINT16 ix;       /* a counter */
   int    retVal;   /* holder for return value */
...
LOCAL STATUS miiAutoNegotiate (PHY_INFO*  pPhyInfo, /* pointer to PHY_INFO structure */
                               UINT8 phyAddr        /* address of a PHY */
                               )
{
   UINT8  regAddr;         /* PHY register */
...
LOCAL STATUS miiAutoNegStart (PHY_INFO*  pPhyInfo, /* pointer to PHY_INFO structure */
                              UINT8 phyAddr        /* address of a PHY */
                              )
{
   UINT16 data;         /* data to be written to the control reg */
...
LOCAL STATUS miiBasicCheck (PHY_INFO*  pPhyInfo, /* pointer to PHY_INFO structure */
                            UINT8 phyAddr        /* address of a PHY */
                            )
{
   UINT8  regAddr;      /* PHY register */
   ...
   UINT16 ix = 0;       /* a counter */
   int    retVal;       /* convenient holder for return value */
   int    maxDelay;     /* maximum delay tick or count */
   int    delayUnit;    /* delay unit/tick per call phyDelayRtn */
   int    delayCount;   /* delay number/tick */
   ...
   /*
    * the old algorithm assumes the delay routine is taskDelay(), which
    * is not appropriate. However, for backward compatibility reason,
    * the old algorithm is still kept if the delay function is taskDelay().
    */

   if (pPhyInfo->phyDelayRtn == (FUNCPTR)taskDelay)
   {
      maxDelay   = pPhyInfo->phyMaxDelay * pPhyInfo->phyDelayParm;
      delayUnit  = sysClkRateGet ();
      delayCount = delayUnit;
   }
   else
   {
      maxDelay   = pPhyInfo->phyMaxDelay;
      delayUnit  = pPhyInfo->phyDelayParm;
      delayCount = 1;
   }
...
LOCAL STATUS miiFlagsHandle (PHY_INFO*  pPhyInfo, /* pointer to PHY_INFO structure */
                             UINT8 phyAddr        /* address of a PHY */
                             )
{
...
LOCAL STATUS miiPhyUpdate (PHY_INFO*  pPhyInfo, /* pointer to PHY_INFO structure */
                           UINT8 phyAddr        /* address of a PHY */
                           )
{
   UINT16 phyStatus;     /* holder for the PHY status */
...
STATUS miiAnCheck (PHY_INFO*  pPhyInfo, /* pointer to PHY_INFO structure */
                   UINT8 phyAddr        /* address of a PHY */
                   )
{
   UINT8  regAddr;      /* PHY register */
...
LOCAL STATUS miiModeForce (PHY_INFO*  pPhyInfo, /* pointer to PHY_INFO structure */
                           UINT8 phyAddr        /* address of a PHY */
                           )
{
   UINT16 data;         /* data to be written to the control reg */
...
LOCAL STATUS miiDefForce (PHY_INFO*  pPhyInfo, /* pointer to PHY_INFO structure */
                          UINT8 phyAddr        /* address of a PHY */
                          )
{
   UINT16 data;         /* data to be written to the control reg */
...
LOCAL STATUS miiAbilFlagSet (PHY_INFO*  pPhyInfo, /* pointer to PHY_INFO structure */
                             UINT8 phyAddr        /* address of a PHY */
                             )
{
   UINT8  regAddr;      /* PHY register */
...
void miiPhyOptFuncMultiSet (PHY_INFO* pPhyInfo, /* device specific pPhyInfo pointer */
...
LOCAL STATUS miiPhyListAdd (PHY_INFO*  pPhyInfo       /* pointer to PHY_INFO structure */
                            )
{
   MII_PHY_NODE* pMiiPhyNode = NULL;    /* pointer to phy node */
...
STATUS miiRegsGet (PHY_INFO* pPhyInfo, /* pointer to PHY_INFO structure */
...
/* FORMAT HASH 44d821df699001bf4fe801efcf7d9e9e */
