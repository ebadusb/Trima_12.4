 ...
 *
 * TITLE:      plsrinsebackno2.cpp
 *
 */

#include "cobeconfig.h"
#include "procdata.h"
#include "plsrinsebackno2.h"
#include "volumeacc.h"

DEFINE_STATE(PlsRinsebackNo2);

PlsRinsebackNo2::PlsRinsebackNo2()
   : StateAbs(),
     _LogFlag(true),
     _Ratio(0.0)
{}
...
int PlsRinsebackNo2::transitionStatus ()
{
   ProcData pd;

   if ( pd.PlasmaRinseback().Status.Get() & ProcPlsRinseback_CDS::No2 )
   {
      return 1;
   }
...
int PlsRinsebackNo2::preProcess ()
{
   ProcData pd;
   ...
   if ( _LogFlag )
   {
      DataLog(log_level_proc_rinseback_info) << "RinsebackNo2 started at"
                                             << " Qrp=" << Qreturn
...
int PlsRinsebackNo2::postProcess ()
{
   ProcData pd;
   ...
   {
      DataLog(log_level_proc_rinseback_info) << "PlsRinsebackNo2 ended at max volume of " << pd.Volumes().VMaxPltRinseback << endmsg;
      _TransitionStatus = 1;
      finished          = true;
   }
   ...
   if ( pd.SafetyControlReservoirIsEmpty() )
   {
      DataLog(log_level_proc_rinseback_info) << "PlsRinsebackNo2 ended with reservoir empty" << endmsg;
      _TransitionStatus = 1;
      finished          = true;
   }
   ...
   if ( finished )
   {
      ...
      pd.PlasmaRinseback().Status.Set( (ProcPlsRinseback_CDS::PlsRinsebackSubstateComplete)(status | ProcPlsRinseback_CDS::No2) );
   }

   return NORMAL;
}

int PlsRinsebackNo2::preEnter ()
{
   ProcData pd;
...
/* FORMAT HASH 53ffcca3401e9cfe6cd81da6cf260551 */
