...
public:
   ...
   int Line (int ln)
   {
      return m_nLineNum = ln;
   }
   LINETYPE Type (void) const
   {
      return m_nType;
   }
   char* pData (void)
   ...
   {
      return( m_nType == LT_SECTION );
   }
   bool IsValue (void) const
   {
      return( m_nType == LT_VALUE );
   }

   // name and value access if appropriate
   const char* cpName (void) const
   {
      return m_pName;
   }
   const char* cpValue (void) const
   {
      return m_pValue;
   }
...
public:
   ...
   CDatFileReader(const char* fname, bool logErrors = false);
   CDatFileReader(const char* fname, bool logErrors, bool throwAwayErrors);
   CDatFileReader(bool logErrors = false);
   virtual ~CDatFileReader();

   // call with filename to read in the file
   DFR_ERROR Initialize (const char* fname, bool logErrors = false);

   // access to most recent error
   DFR_ERROR Error (void) const
   {
      return m_nError;
   }
   ...
   std::string GetString (const char* section, const char* name);
   ...
   // write the text file out to the filename
   // standard single file write using FILE *
   // Provide complete file/path as input.
   bool Write (const char* fname);

   // For configuration files (config.dat, hw.dat ,...)
   // Provide just the filename from filenames.h
   // Performs a single block write, updates the crc
   // file, writes the backup file and the backup crc
   // file.  Directories specified in filenames.h
   bool WriteCfgFile (const char* fname);

   bool HasSection (const char* sct);
protected:
   ...
   void Erase (void);
   void Init (bool logErrors, bool throwAwayErrors = false);
   void SetError (DFR_ERROR err);
   void LogErr (const char* info, const char* line) const;
   void LogErr (const char* line, const char* valstr, float mn, float mx) const;
...
private:
   // not implemented
   CDatFileReader(const CDatFileReader&);
   CDatFileReader& operator = (const CDatFileReader&);
};

#endif

/* FORMAT HASH 8814a9cd9220a7b4d75afd882e2fde5f */
