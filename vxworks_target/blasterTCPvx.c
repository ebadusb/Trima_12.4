/* $Id: blasterTCPvx.c 1.1 2007/11/08 00:59:22Z dslausb Exp $ */
...
void blasterTCP (char *targetAddr, int port, int size, int blen, int zbuf);
/*****************************************************************
 *
...
void blasterTCP (char *targetAddr, int port, int size, int blen, int zbuf)
	...
	if (quitFlag == 1)
	    break;
        if (zbuf)
            nsent = zbufSockBufSend(s, buffer, size, NULL, 0, 0);
        else
