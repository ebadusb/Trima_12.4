...
#   else /* LOCAL_MEM_LOCAL_ADRS is 0x0 */
    ...
    VM_STATE_FOR_MEM_APPLICATION
	 ...
	 /* upper memory for internal logging */
	 {
    (void *) SYSTEM_RAM_SIZE,
    (void *) SYSTEM_RAM_SIZE,
    0x00200000,
    VM_STATE_MASK_FOR_ALL,
    VM_STATE_FOR_MEM_APPLICATION
...
#   endif /* (LOCAL_MEM_LOCAL_ADRS >= VM_PAGE_SIZE) */
    ...
    /* ISA memory mapping for Ocelot - unused for other processors */
    {
    (void *) 0xffd00000,
    (void *) 0xffd00000,
    0x10000,
...
#else
#   include "intrCtl/i8259Intr.c"
#   include "os/i8253Timer.c"	/* includes timestamp driver */
#endif	/* defined(VIRTUAL_WIRE_MODE) */

#ifdef	INCLUDE_PCI                     /* BSP PCI bus & config support */

/* PCI related functions defined by config files */
VOID sysPciCfgInit(int mechanism, UINT32 addr1, UINT32 addr2, UINT32 addr3);
VOID sysPciIntInit(void);
int sysPciIvecToIrq(int vector);
VOID pciInt(int irq);
STATUS pciIntDisconnect2(VOIDFUNCPTR *vector, VOIDFUNCPTR routine, int parameter);
STATUS pciConfigReset(int startType);
...
STATUS pciConfigStatusWordShow(int bus, int device, int function, void *pArg);
STATUS pciConfigCmdWordShow(int bus, int device, int function, void *pArg);
STATUS pciConfigFuncShow(int bus, int device, int function, void *pArg);
