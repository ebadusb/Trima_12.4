...
--------------------
03p,26jun03,rae  added TELNET_ANVL_FIX conditional
03o,07apr03,elr  prevent hang condition during dropped connection (SPR #85916)
					  prevent hang condition during login (SPR #86154)
					  addtional checks for errors during socket writes
					  remove i960 references
...
/* jd11007 - Added this printf so that log trap write would catch any TELNETD_DEBUG msgs... */
...
int telnetTaskPriority  = 99;		/* priority of telnet tasks */
...
LOCAL TELNETD_TASK_DATA * telnetdTaskList = NULL;
...
LOCAL SEM_ID telnetdMutexSem = NULL;
		...
		)
{
	...
	if ( staticFlag )
	{
		if ( telnetdParserControl == NULL )
		{
			...
			if ( (telnetdParserControl == (FUNCPTR) shellParserControl ) &&    
				(numClients > 1) )
		{
			TELNETD_DEBUG ("telnetd:  vxWorks shell does not support multiple static initializations!  errno:%#x\n", 
					errno, 0, 0, 0, 0, 0);
			return(ERROR);
		}
	}
		...
		)
{
	...
	if ( listen (telnetdServerSock, 1) < 0 )
	{
		close (telnetdServerSock);
		if ( telnetdTaskFlag )
			telnetdTaskDelete (telnetdMaxClients);
		...
		)
{
	...
	for ( count = 0; count < numTasks; count++ )
	{
		...
		if ( telnetdTaskList [count].pSession != NULL )
		{
			telnetdSessionDisconnect (telnetdTaskList [count].pSession, FALSE);
			free (telnetdTaskList [count].pSession);
			telnetdTaskList [count].pSession = NULL;
		}
	}
...
LOCAL TELNETD_SESSION_DATA *telnetdSessionAdd (void)
{
	TELNETD_SESSION_DATA *     pSlot = NULL;
	int count = 0;
	...
	{
		pSlot = (TELNETD_SESSION_DATA *) calloc (sizeof (TELNETD_SESSION_DATA), 1);
		if ( pSlot == NULL )
		{
			semGive (telnetdMutexSem); 
			return(NULL);
		...
		)
{
	...
	else
	{
		...
		/* 
		 * Re-Initialize all elements of the structure to recognizable values 
		 */
...
*
* If the user terminates the connection, then telnetInTask will submit this
* function as a job to the excTask.
*
* RETURNS N/A
*
* NOMANUAL
...
void telnetd (void)
{
	...
	FOREVER
	{
		...
		printf("telnetd: connection attempt from %s\r\n", inet_ntoa(clientAddr.sin_addr));
		...
		/* if tShell has been suspended, let's restart it... */
		tShellId = taskNameToId("tShell");
		if (tShellId != ERROR)
		{
			if (taskIsSuspended(tShellId) == 1)
			{
				taskDelete(tShellId);
		...
		/* If we haven't created the pseudo tty device, do so now */
		...
		if ( telnetdTaskFlag )
		{
			...
			pSlot->startOutputFlag = TRUE;
			sprintf(buf,"\n");   
			write (pSlot->slaveFd, buf, strlen(buf));
		}
...
#ifndef TELNET_ANVL_FIX
		if ( localDoOpt (pSlot->slaveFd, TELOPT_ECHO, TRUE, newSock, FALSE) 
				== SOCKET_ERROR )
		{
			/* socket failure during during negotiation.   Close down and restart */
			telnetdSessionDisconnect (pSlot, TRUE);
			continue;
		}
#endif
		...
		)
{
	...
	{
		if ( pSlot->outputFd == -1 )	  /* check for first time */
		{
			semTake (pSlot->startOutput, WAIT_FOREVER);
		}
		else
		{
			while ( pSlot->startOutputFlag == FALSE )
			{
				/* flush any spurious output */

				n = read (pSlot->outputFd, buf, sizeof (buf));
			}
			semTake (pSlot->startOutput, NO_WAIT);
		}
	}
		...
		)
{
	...
	/* Wait until the login has finished */

	while ( pSlot->remoteStarted == FALSE )
		taskDelay(1);
		...
		)
{
	...
	while ( --n >= 0 )
	{
		...
		ci = (unsigned char) cc;	/* 
											 * convert to int since many values
		...
		switch ( state )
		{
						...
						case AYT:		  /* Are You There? */
							{
								...
								expectedBytes = (sizeof (aytAnswer) - 1);

								if ( write (clientSock, aytAnswer, expectedBytes) != expectedBytes )
								{
									/* 
									 * socket failure during during negotiation.   
			...
			case TS_WILL:			/* remote side said it will do opt */
				if ( remDoOpt (slaveFd, ci, TRUE, clientSock, TRUE) == SOCKET_ERROR )
				{
					/* 
					 * socket failure during during negotiation.   
					 * Close down and restart 
			...
			case TS_WONT:			/* remote side said it wont do opt */
				if ( remDoOpt (slaveFd, ci, FALSE, clientSock, TRUE) == SOCKET_ERROR )
				{
					/* 
					 * socket failure during during negotiation.   
					 * Close down and restart 
			...
			case TS_DO:				  /* remote wants us to do opt */
				if ( localDoOpt (slaveFd, ci, TRUE, clientSock, TRUE) == SOCKET_ERROR )
				{
					/* 
					 * socket failure during during negotiation.   
					 * Close down and restart 
			...
			case TS_DONT:			/* remote wants us to not do opt */
				if ( localDoOpt (slaveFd, ci, FALSE, clientSock, TRUE) == SOCKET_ERROR )
				{
					/* 
					 * socket failure during during negotiation.   
					 * Close down and restart 
		...
		)
{
	...
	if ( (remOpts [opt] != doOpt) || remFlag )
	{
		...
		bytesWritten = write (clientSock, msg, 3);

		if ( bytesWritten < 3 )
			return(SOCKET_ERROR); /* Shut down this session */
		...
		)
{
	...
	if ( (myOpts [opt] != will) || remFlag )
	{
		...
		if ( write (clientSock, msg, 3) == ERROR )
			return(SOCKET_ERROR);
