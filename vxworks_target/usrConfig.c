...
19z,23sep03,nrv  Removing code for sys(Ohci/Ehci/Uhci)PciInit
19y,17sep03,nrv  Updating for USB2 stack
...
)
{
...
#ifdef CONTROL_BUILD
   trimaSysClockInit();
   sysClkConnect ((FUNCPTR) trimaSysClock, 0); /* connect clock ISR */
#endif
   ...
   logMsg ("logging started to %s [%d], queue size %d\n",(int)consoleName,consoleFd,MAX_LOG_MSGS,4,5,6);
   ...
   {                                               /* initialize hard disk driver */
      ...
      ATA_RESOURCE *pAtaResource = NULL;
   ...
   win_ReleaseMutex(simUpMutex);
    ...
    /* Perform platform and node dependent initialization */
   platformInit();
}
