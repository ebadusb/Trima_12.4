...
int mqPxLibInit
    ...
    if (!mqLibInstalled)
        ...
        if ( (mqNameTbl = symTblCreate(hashSize, FALSE, memSysPartId)) != NULL &&
             classInit(mqClassId, sizeof (struct mq_des), OFFSET(struct mq_des, f_objCore), NULL, NULL, NULL) == OK &&
             (mqNameSem = semMCreate(SEM_Q_PRIORITY | SEM_INVERSION_SAFE)) != NULL )
            mqLibInstalled = TRUE;
        }
...
mqd_t mq_open
    ...
    if ( taskPriorityGet( 0, &oldPriority ) != OK )
        return ((mqd_t) -1);

    if ( taskPrioritySet( 0, 25 /*highest priority for message queue tasks*/ ) != OK )
        {
    ...
    semTake (mqNameSem, WAIT_FOREVER);
