...
/* Import the following for the network auto detect */
...
STATUS sys557PciInit
    ...
    /* Find an empty slot in endDevTable and update it
	    for the network auto detection. */
    for(count = 0, pDevTbl = endDevTbl; count < 4; pDevTbl++, count++)
    {
        if( pDevTbl->endLoadFunc == END_TBL_END )
        {
            pDevTbl->unit = count;
            pDevTbl->endLoadFunc = fei82557EndLoad;
            pDevTbl->endLoadString = "-1:0x00:0x20:0x20:0x00";
            pDevTbl->endLoan = TRUE;
            pDevTbl->pBSP = NULL;
            break;
        }
    }
