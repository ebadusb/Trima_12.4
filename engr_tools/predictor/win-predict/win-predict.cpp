...
void printHelp(void);
void readSWDat(void);
extern int trimaSysLoadOptionFiles();
...
{  
	...
	case 'm': /* Amap Platelets */ 
		{ 
			if(argc != 6) {
			   cout << "usage: predict m donor_pool config_dat cobe_config output_file" << endl;
			   exit(0);
			}
	...
	case 's': 
		...
		break;
	case 'r': //repredict throughout
	   {
			if(argc != 5) {
			   cout << "usage: predict r dlog cobe_config procedureNum" << endl;
			   exit(0);
			}
		   analyzer = new Analyzer(argv[2], argv[3]);
		   analyzer->AnalyzeEntireRun(atoi(argv[4]));
		}
		break; 
	default:
		{
...
void printHelp(void)
{
	...
	cout << "\n\tRepredict Throughout:\n\tpredict r dlog cobe_config procedureNum" << endl; 
}
