...
public:
   ...
   bool init( const SendType dt ) { UNUSED_ARGUMENT(dt);return true;} ;
   //
   // Initialize the message for sending and receiving
   bool init( const CallbackBase &cb, const SendReceiveType dt=SNDRCV_RECEIVE_ONLY ) {UNUSED_ARGUMENT(cb); UNUSED_ARGUMENT(dt);return true; };

   //
   // Register the message for reception 
   void registerMsg( const CallbackBase &cb ) {UNUSED_ARGUMENT(cb);};
   //
