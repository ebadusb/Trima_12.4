# FILENAME: C-HYP-1.001d.py
# PURPOSE: .
scriptName = "C-HYP-1.001d.py"
...
def load_and_run(runner):
    ...
    TBVLimit = float(raw_input('Enter the "TBV(%)" setting from the "Donor volume limits" configurations: '))
    setTBV = float(raw_input('Enter the TBV [ml] displayed by Trima at donor selection: '))
  
    limit = setTBV * TBVLimit /100             #The product volume that can be removed from the donor before the TBV% limit is exceeded
    offset = 15   # Volume [ml] that the spoof will undershoot and overshoot the configured volume limit by
    ACvol = 50    # Spoofed volume [ml] of AC in each product bag
    ...
    runner.AddCommand('spoof_main "-object ProcVolumes_ACinPlasmaBag -data data -value '+ str(ACvol) +'"\n','->',False)
    runner.AddCommand('spoof_main "-object ProcVolumes_ACinPlateletBag -data data -value '+ str(ACvol) +'"\n','->',False)
    runner.AddCommand('spoof_main "-object ProcVolumes_ACinRBCBag -data data -value '+ str(ACvol) +'"\n','->',False) 
    runner.AddCommand('spoof_main "-object ProcVolumes_Vac -data data -value 220"\n','->',False) # spoof 220 mL AC accum to get away from safety side mimic	
    runner.AddCommand('spoof_main "-object ProcVolumes_PlateletVolume -data data -value '+ str((limit/3) + ACvol - offset/3) +'"\n','->',False)
    runner.AddCommand('spoof_main "-object ProcVolumes_RBCVolume -data data -value '+ str((limit/3) + ACvol - offset/3) +'"\n','->',False)
    runner.AddCommand('spoof_main "-object ProcVolumes_PlasmaVolume -data data -value '+ str((limit/3) + ACvol - offset/3) +'"\n','->',False)
    ...
    time.sleep(10)
    runner.AddCommand("Log_Test_Info(\"Collected product volume spoofed below limit. Total Product Volume: " + str(limit + (3*ACvol) - offset) + " Total AC Volume: " + str(3*ACvol) +"\")\n", "->", False) 
    ...
    print 'Pausing for 10 seconds...'
    time.sleep(10)
    runner.AddCommand("Log_Test_Info(\"Collected product volume spoofed above limit. Total Product Volume: " + str(limit + (3*ACvol) + offset) + " Total AC Volume: " + str(3*ACvol) +"\")\n", "->", False) 
