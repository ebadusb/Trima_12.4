...
raw_input("\nWhen Blood Prime starts, press ENTER to run Part 1.");
...
raw_input("\nWhen collection begins, press ENTER to run Part 2.");
#os.system('cls')
...
runner.AddCommand('spoof_main "-enable"\n','->',False)
runner.AddCommand('spoof_main "-object ProcRun_IntegratedPlateletYield -data data -value 120000000000 -trace"\n','->',False)

runner.Run()
runner.ResetCommands()

raw_input("\nOnce you see platelets being collected on the RUN screen, press ENTER to spoof the valve error.");
#os.system('cls')
print('Spoofing valve error.')

runner.AddCommand('spoof_main "-id Spoof_HWFault -var1 valveFault -var2 HW_FAULT -trace"\n','->',False)
