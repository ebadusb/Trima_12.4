# FILENAME: C-PCM-1.001a.py
# PURPOSE: .
scriptName = "C-PCM-1.001a.py"
...
def load_and_run(runner):
	...
	curFlowRate = raw_input("\n\nA few seconds after the beginning of the second draw cycle,\nenter the Inlet pump flow rate, then press enter: ")


	runner.AddCommand("Log_Test_Info(\"Running spoof script " + scriptName + "\")\n", "->", False)

	if float(curFlowRate) < 85:
		runner.AddCommand('spoof_main "-object CHwStates -data inletRPM -mult 1 -offset 5.1 -enable -trace"\n','->',False)
	else:
		runner.AddCommand('spoof_main "-object CHwStates -data inletRPM -mult 1.06 -offset 0 -enable -trace"\n','->',False)
