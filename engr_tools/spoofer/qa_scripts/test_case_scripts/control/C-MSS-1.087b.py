# FILENAME: C-MSS-1.087b.py
# PURPOSE: .
scriptName = "C-MSS-1.087b.py"
...
def load_and_run(runner):
    ...
    #Inputting the CPS calibration and PTF Baseline
    Cal = float(raw_input('Enter the CPS Calibration (CPS_Cal): '))
    Limit = 1100 +Cal
    BaselineTarget = 980+Cal
    Margin=2
    
    #RBC1-------------------------------------------------------------------------------------------------------------------
    
    #Spoofing RBC1 Baseline within 455 mmHg of 1100 
    ...
    #Spoofing 5 second CPS spike of (1102), then back to the baseline
    raw_input("\nPress ENTER to spoof a CPS high during RBC1.");
    runner.AddCommand('spoof_main "-object HalStatus_cps -data data -value '+ str(Limit+Margin) +'"\n','->',False)
    runner.Run()
    runner.ResetCommands()
    time.sleep(5)
    runner.AddCommand('spoof_main "-object HalStatus_cps -data data -value '+ str(BaselineTarget) +'"\n','->',False)
    ...
    #Spoofing 5 second CPS spike of (1102)
    ...
    #Spoofing 5 second CPS spike of (1102), then back to the baseline
    raw_input("\nPress ENTER to spoof a CPS high during RBC2.");
    runner.AddCommand('spoof_main "-object HalStatus_cps -data data -value '+ str(Limit+Margin) +'"\n','->',False)
    runner.Run()
    runner.ResetCommands()
    time.sleep(5)
