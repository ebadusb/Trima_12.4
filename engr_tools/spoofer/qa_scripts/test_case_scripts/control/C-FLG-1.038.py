...
# PART 2 - spoof the collect flow so that Qch falls temporarily between 0.1 and 1.2 (Qch = Qc when plasma valve is open)
raw_input("Press ENTER to run Part 2.");
os.system('cls')
print('Running Part 2')

runner.AddCommand('spoof_main "-object HalStatus_plateletCmdFlow -data data -value 0.8"\n','->',False)
runner.AddCommand('spoof_main "-enable"\n','->',False)

runner.Run()
runner.ResetCommands()

# PART 3 - first, end the Qch spoof, then spoof red/green values during collect substates to avoid unwanted flags
raw_input("Press ENTER to run Part 3.");
os.system('cls')
print('Running Part 3')

runner.AddCommand('spoof_main "-unspoof -object HalStatus_plateletCmdFlow -data data"\n','->',False)
...
#Optional prompt to notify operator that script and all spoofs have been executed successfully.
