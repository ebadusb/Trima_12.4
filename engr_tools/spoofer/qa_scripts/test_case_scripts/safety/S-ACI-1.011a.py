# FILENAME: S-ACI-1.011a.py
# PURPOSE: .
scriptName = "S-ACI-1.011a.py"
...
def load_and_run(runner):
    ...
    runner.AddCommand('spoof_main "-object CHwOrders -data returnRPM -value 120 -trace"\n','->',False)
    runner.AddCommand('spoof_main "-object CHwOrders -data plateletRPM -value 120"\n','->',False)
    ...
    #**************************************************************************************************************************
    raw_input("After pressing 'Saline Bolus' button, press ENTER to continue:")
    raw_input("Once Return pump starts pumping saline bolus to donor, press ENTER:")
    ...
    #Example command: runner.AddCommand('spoof_main "-enable -object CHwOrders -data acRPM -value 0.33 -enable"\n','->',False)
    #**************************************************************************************************************************
    runner.AddCommand("Log_Test_Info('Accelerating the bolus delivery; Spoofing the return pump and collect pump RPM to 120 on a 3.5/1.5 duty cycle')\n", "->", False)
    runner.AddCommand('spoof_main "-id EnableDisable -timeout 360"\n','->',False)
