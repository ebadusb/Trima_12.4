...
tbv_L = float(raw_input("Enter 'Set TBV' value from dlog: "))
tbv_mL = tbv_L*1000
wb_limit = tbv_mL * 2.2
inlet_stroke_vol = 0.8043483 #20mmHg APS, 40 RPM cmdSpd 
spoof_revs = int((wb_limit)/inlet_stroke_vol)

runner.AddCommand('spoof_main "-object CHwStates -data inletRevs -value '+str(spoof_revs)+'"\n','->',False)

runner.Run()
runner.ResetCommands()

raw_input('At the beginning of first draw, press ENTER: ')
runner.AddCommand('spoof_main "-enable"\n','->',False)
runner.Run()
runner.ResetCommands()
#(Optional) Pause here with additional details if necessary (i.e. instructions, timing, etc.).
