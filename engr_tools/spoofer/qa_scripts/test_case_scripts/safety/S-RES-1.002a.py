...
raw_input("Press enter to send safety reservoir event messages:")
runner.AddCommand('spoof_main "-enable"\n','->',False)
for i in range(5):
	#runner.AddCommand("Log_Test_Info(\"Running spoof script " + scriptName + "\")\n", "->", False)
	runner.AddCommand('spoof_main "-object SHwStates -data reservoir -value SHW_RESERVOIR_LOW"\n','->',False)
	runner.AddCommand('spoof_main "-unspoof -object SHwStates -data reservoir"\n','->',False)
