...
/*
   ...
   if (_handle->_mutexSemaphore == NULL)
   {
      ...
      DataLog(log_level_cds_debug) << "_mutexSemaphore value(" << std::hex << _handle->_mutexSemaphore << std::dec << ")." << endmsg;
   ...
   if (_handle->_readSemaphore == NULL)
   {
      ...
      DataLog(log_level_cds_debug) << "_readSemaphore value(" << std::hex << _handle->_readSemaphore << std::dec << ")." << endmsg;
   ...
   if (_handle->_writeSemaphore == NULL)
   {
      ...
      DataLog(log_level_cds_debug) << "_writeSemaphore value(" << std::hex << _handle->_writeSemaphore << std::dec << ")." << endmsg;
*/
   //
...
unsigned int DataStore::SetAllPfData ( )
{
   ...
   DataLog( log_level_cds_debug ) << "Datastore std::list " << &_pfrDataStoreList << endmsg;
...
//   DataLog( log_level_cds_debug ) << "Add element to datastore " << _name << endmsg;
   ...
   //  is the first time adding an element to the pfr std::list.
   //
...
unsigned int DataStoreSymbolContainer::SetPfData( unsigned char *memBlock )
{
   ...
   for (ELEMENT_LISTTYPE::iterator pfrListIterator = _pfrList.begin(); pfrListIterator != _pfrList.end(); ++pfrListIterator)
   {
      ...
      DataLog(log_level_cds_debug) << "element " << count << " saved " << dataSize << " bytes at " << std::hex << memBlock+size << endmsg;
      size += dataSize;
   }
   Unlock( ROLE_RO );
...
#if 0
   case ITEM_DATA:
      std::cout << "DataItem " << name << ",\tvalue " << *data << std::endl;
      break;

   case ITEM_SPOOF:
      if (dump_table_choice > 0)
         std::cout << "SpoofItem " << name << ",\tvalue " << *data << std::endl;

      break;

   case ITEM_PFR_LIST:
      if (dump_table_choice > 0)
         std::cout << "PFRList " << name << ",\tptr " << data << std::endl;

      break;

   case ITEM_MUTEX_SEMAPHORE:
      if (dump_table_choice > 0)
         std::cout << "MUTEX " << name << ",\tptr " << data << std::endl;

      break;

   case ITEM_READ_SEMAPHORE:
      if (dump_table_choice > 0)
         std::cout << "READSEM " << name << ",\tptr " << data << std::endl;

      break;

   case ITEM_WRITE_SEMAPHORE:
      if (dump_table_choice > 0)
         std::cout << "WRITESEM " << name << ",\tptr " << data << std::endl;

      break;

   case ITEM_SIGNAL_READ:
      if (dump_table_choice > 0)
         std::cout << "SigRead " << name << ",\tvalue " << *data << std::endl;

      break;

   case ITEM_SIGNAL_WRITE:
      if (dump_table_choice > 0)
         std::cout << "SigWrite " << name << ",\tvalue " << *data << std::endl;

      break;

   case ITEM_READ_COUNT:
      if (dump_table_choice > 0)
         std::cout << "ReadCount " << name << ",\tvalue " << *data << std::endl;

      break;

   case ITEM_WRITER_DECLARED:
      if (dump_table_choice > 0)
         std::cout << "WriterDeclared " << name << ",\tvalue " << *data << std::endl;

      break;

   case ITEM_SPOOF_CACHE:
      if (dump_table_choice > 0)
         std::cout << "SpoofCache " << name << ",\tvalue " << *data << std::endl;
    
      break;
#endif

   default:
      std::cout << "Symbol " << name << ", val " << data << ", type " << (int)type
