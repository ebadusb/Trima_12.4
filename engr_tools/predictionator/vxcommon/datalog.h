 ...
 *
 * $Header: //bctquad3/home/BCT_Development/Trima5.R/Trima/engr_tools/predictionator/vxcommon/rcs/datalog.h 1.5 2008/07/29 03:14:04Z ssunusb Exp $
 * $Log: datalog.h $
 * Revision 1.5  2008/07/29 03:14:04Z  ssunusb
 * moved to std c++ headers
 * Revision 1.4  2007/07/24 16:13:45Z  jheiusb
 * Revision 1.3  2007/07/24 16:11:01Z  jheiusb
 * time dependant log name so no overwriting
 * Revision 1.2  2007/07/19 19:49:27Z  ssunusb
 * Moved datalog changes from 5.2 to 5.R. (support for logging).
...
static DataLog_Result datalog_SetTaskOutputOptions(DataLog_TaskID task, DataLog_EnabledType log, DataLog_ConsoleEnabledType console) { return DataLog_OK;};
...
class DataLog_Stream : public std::ostringstream
{
	friend class DataLog_OutputBuffer;
	friend class DataLog_CriticalBuffer;
//	friend std::ostream & endmsg(std::ostream & stream);
...
public:
	DataLog_Level(void) {};
   DataLog_Result initialize(const char * levelName) {return DataLog_OK;};
	...
	std::ostream &operator()(const char * fileName, int lineNumber) 
   {
      ...
      buf << "D:\\rundata\\temp\\Predictionator_5R_log_" 
	   ...
	   {
		   p_logfile = &logfile; 
		   logfile.open( buf.str().c_str() , std::ios::app);
	   }
	   logfile.flush();
		return logfile;
   };
...
//std::ostream & endmsg(std::ostream & stream);
