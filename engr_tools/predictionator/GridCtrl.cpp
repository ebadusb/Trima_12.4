...
CCellID CGridCtrl::SetFocusCell(CCellID cell)
{
    ...
    if (IsValid(m_idCurrentCell))
    {
        ...
        // EFW - New addition.  If in std::list mode, make sure the selected
                                 ...
                                 BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
    ...
    // If we are selecting cells, then first clear out the std::list of currently selected cells, then
    if (bSelectCells)
    {
        POSITION pos;
        ...
        // (eg Ctrl is being held down) then copy them to the newly created std::list, and mark 
        // all these cells as selected
        // Note that if we are std::list mode, single row selection, then we won't be adding 
...
void CGridCtrl::OnSelecting(const CCellID& currentCell)
{
    ...
    // EFW - Bug fix [REMOVED CJM: this will cause infinite loop in std::list mode]
...
/*virtual*/ void CGridCtrl::DestroyCell(int nRow, int nCol)
{
    ...
    // will remove the cell from the selected std::list.
                          ...
                          int low, int high)
{
    ...
    // loop through the std::list until indices cross
    while (lo <= hi)
    {
...
// a variable argument std::list too.
...
BOOL CGridCtrl::SetItemState(int nRow, int nCol, UINT state)
{
    ...
    // If the cell is being unselected, remove it from the selected std::list
    ...
    // If cell is being selected, add it to the std::list of selected cells
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    else if (IsCellSelected(m_LeftClickDownCell))
    {
        ...
        // If control is pressed then unselect the cell or row (depending on the std::list mode)
