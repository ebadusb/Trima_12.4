...
public:
   ...
   bool initBlocking( const unsigned int qSize=MessageSystemConstant::DEFAULT_Q_SIZE ) { UNUSED_ARGUMENT(qSize);return true; }; // uses default task name
   bool initBlocking( const char *qname, const unsigned int qSize=MessageSystemConstant::DEFAULT_Q_SIZE ){ UNUSED_ARGUMENT(qname);UNUSED_ARGUMENT(qSize);return true; };
   bool initNonBlock( const unsigned int qSize=MessageSystemConstant::DEFAULT_Q_SIZE ){ UNUSED_ARGUMENT(qSize);return true; }; // uses default task name
   bool initNonBlock( const char *qname, const unsigned int qSize=MessageSystemConstant::DEFAULT_Q_SIZE ){ UNUSED_ARGUMENT(qname);UNUSED_ARGUMENT(qSize);return true; };
